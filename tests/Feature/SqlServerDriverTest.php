...
class SqlServerDriverTest extends TestCase
{
    ...
    public function test_zero_threshold_returns_anti_correlated_results(): void
    {
        $post1 = Post::create(['title' => 'Laravel', 'body' => 'PHP Framework']);
        $post2 = Post::create(['title' => 'Django', 'body' => 'Python Framework']);

        $v1 = $this->padVector([1.0, 0.0, 0.0]);
        $v2 = $this->padVector([-1.0, 0.0, 0.0]);

        $this->setVector($post1, $v1);
        $this->setVector($post2, $v2);

        $results = Post::similarTo($v1, limit: 10, threshold: 0.0);

        $this->assertCount(2, $results);
        $this->assertEquals($post2->id, $results->last()->id);
        $this->assertLessThan(0.0, $results->last()->similarity_score);
    }
